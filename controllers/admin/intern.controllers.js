const Intern = require("../../models/internSubscription.model");
...
exports.getInterns = async (req, res) => {
    try {
        const data = await Intern.find().sort({ createdAt: -1 })
        ...
        return res.status(200).json({ message: "Intern records retrieved successfully", data })
    }
    catch (err) {
...
exports.getIntern = async (req, res) => {
    try {
        const data = await Intern.findById(req.params.internId)
        if (!data) {
            return res.status(404).json({ message: "Intern record not found" })
        }
        return res.status(200).json({ message: "Intern record retrieved successfully", data })
    }
    catch (err) {
        console.error(err.message)
        return res.status(500).json({ message: "Server error, try again later" })
